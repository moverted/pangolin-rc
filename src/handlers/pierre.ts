...
pierreRoutes.post('/chat', async (c) => {
  ...
  // marathon context is irrelevant to outreach drafting, so skip those reads when it's live.
  ...
  // claiming he can't. Empty string when they have none (or in outreach mode); the grounding
  // rule handles the empty case.
  const marathon = outreachOk ? '' : email ? await marathonBlock(c.env, email) : '';
