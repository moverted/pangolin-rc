...
-- rows first. Owner = edward.m.willett@gmail.com (a real local user).
--
...
DELETE FROM coviewer WHERE owner_email = 'edward.m.willett@gmail.com';

INSERT INTO coviewer (id, owner_email, display_name, relationship, linked_email, is_default, created_at) VALUES
  (lower(hex(randomblob(16))), 'edward.m.willett@gmail.com', 'Anne',           'WIFE',          'anne@local.test', 1, CAST(strftime('%s','now') AS INTEGER)*1000),
  (lower(hex(randomblob(16))), 'edward.m.willett@gmail.com', 'Audrey Willett', 'DAUGHTER',      NULL,              0, CAST(strftime('%s','now') AS INTEGER)*1000),
  (lower(hex(randomblob(16))), 'edward.m.willett@gmail.com', 'Bryce Willett',  'SON',           NULL,              0, CAST(strftime('%s','now') AS INTEGER)*1000),
  (lower(hex(randomblob(16))), 'edward.m.willett@gmail.com', 'Rose Reis',      'MOTHER IN LAW', NULL,              0, CAST(strftime('%s','now') AS INTEGER)*1000);
